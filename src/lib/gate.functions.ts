...
2) "usages": このテーマの「使い道」の題材案を必ず10件。各項目 { "title": 20字以内の題材名, "desc": 60〜100字の具体的内容 }。
   ...
   - **同じテーマでも切り口・目的・立場・出力形式を大きく変え、似た内容の題材を並べるのは禁止**。表層的な「○○について書く／振り返る」系ばかりにしない。
   - 以下のカテゴリから **最低5カテゴリ以上をまたいで** 構成する。毎回同じカテゴリ偏重にならないよう、テーマに応じて最も意外性の高い組み合わせを選ぶこと:
     * 日常生活アドバイス系（例: レトロなら「部屋をレトロおしゃれにする購入品アドバイス」、星なら「今夜の観察計画」、睡眠なら「寝起きを良くする環境づくりリスト」）
     * 読み物・物語・エッセイ出力系（例: 星なら「星の神話を中学生にもわかる物語で紹介」、経済なら「歴史上の出来事を登場人物視点で語る」）
     * 学習・教育系（「中学生向けにやさしく解説」「クイズ生成」「学習ロードマップ」「誤概念を直す対話」）
     * 創作・エンタメ系（「短編小説の設定」「詩・俳句」「脚本・台詞」「ゲームシナリオ」）
     * ビジネス・企画系（「新商品コンセプト」「マーケコピー」「プレゼン構成」「顧客ペルソナ作成」）
     * 意思決定・分析系（「複数案比較」「メリデメ整理」「SWOT・PEST」「根拠の可視化」）
     * 対話・シミュレーション系（「専門家との架空対話」「ソクラテス式問答」「面接ロールプレイ」「歴史人物との対話」）
     * 変換・要約系（「専門文献を平易にリライト」「別分野へのアナロジー変換」「箇条書きを文章に」）
     * 内省・自己対話系（任意: 「思考の癖・バイアス指摘」「理想の自己への手紙」「自分の思考体系を模倣した分身との対話」）
     * 文化・歴史・民俗系（「概念にまつわる世界各地の伝承比較」「時代別の受容史」「古語・古典の現代語訳」）
     * 逆説・批判系（「あえて反対の立場から論駁」「悪魔の代弁者」「前提を疑う質問生成」）
     * 健康・ライフスタイル系（「習慣化プラン」「体調と行動の振り返り」「人間関係のコミュニケーション改善」）
   - **意外性を最重視**。表面的な用途（例: 星で「星座を解説」だけ、ジャーナリングで「今日を振り返る」だけ）は禁止。人間が思いつきにくいが検索されうる切り口を必ず半数以上入れる。
